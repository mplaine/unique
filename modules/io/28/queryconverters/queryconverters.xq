...
(: Module :)
module namespace qc                         = "http://queryconverters.28.io/queryconverters";
...
import module namespace uql1-par            = "http://queryconverters.28.io/unique_level_1_parser";
...
declare %private function qc:convert-unique-string-to-xpath-string( $query as xs:string ) as xs:string {
    let $uql1ParserResult                   := uql1-par:parse-selectors_group( $query )
    return
        (: Selectors or UniQue :)
        if ( empty( $uql1ParserResult/self::ERROR ) ) then
            let $unfixedXPath               := string-join( qc:parse-result-tree( $uql1ParserResult ), '' )
...
declare %private function qc:parse-result-tree( $element as element() ) as xs:string* {
    ...
    (: combinator :)
        ...
        for $childElement in $element/element()
            return
                if ( local-name( $childElement ) = "PLUS" or local-name( $childElement ) = "GREATER" or local-name( $childElement ) = "TILDE" or local-name( $childElement ) = "S" ) then
                    qc:parse-result-tree( $childElement )
                else
    ...
    (: S :)
        ...
        if ( local-name( $element/.. ) = "combinator" and not( boolean( $element/preceding-sibling::GREATER ) ) and not( boolean( $element/preceding-sibling::PLUS ) ) and not( boolean( $element/preceding-sibling::TILDE ) ) ) then
            "//"
        else
